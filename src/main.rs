...
fn upload_image_to_files() -> Element {
    ...
    let group_images_from_disks = move |_| {
        let mut dirs: Vec<String>=Vec::new();
        let disks = Disks::new_with_refreshed_list();
        for disk in disks.iter() {
            if disk.is_removable(){
                let dir = disk.mount_point().to_str().unwrap().to_string();
    ...
    rsx! {
        div {
            ...
            div {
                button{
                    class:"btn-blue ml-4",
                    onclick: group_images_from_disks,
                    "refresh",
                }
            }
            div{
            ...
            div {
                ...
                for (group_index, group) in groups.read().iter().enumerate() {
                    div {
                        ...
                        div {
                            class:"m-2 ",
                            for (index,(_filename, _timestamp)) in group.images.iter().enumerate() {
                                if index == 15{
                                   hr {
                                    class:"my-12 h-0.5 border-t-0 bg-neutral-100 dark:bg-white/10",
                                   }
                                }
                            }
                        }
